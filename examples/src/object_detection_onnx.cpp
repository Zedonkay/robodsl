...
object_detectionOnnxManager::object_detectionOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "object_detection_onnx");
}
...
bool object_detectionOnnxManager::initialize() {
    // Set model paths
    object_detection_model_model_path_ = "models/object_detection_model.onnx";
...
bool object_detectionOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    // Run object_detection_model model
    try {
        ...
        std::cerr << "ONNX inference failed for object_detection_model: " << e.what() << std::endl;
        return false;
    }
...
void object_detectionOnnxManager::cleanup() {
    sessions_.clear();
}

bool object_detectionOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load object_detection_model model
    try {
        auto session = std::make_unique<Ort::Session>(env_, object_detection_model_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load object_detection_model model: " << e.what() << std::endl;
        return false;
    }
...
bool object_detectionOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool object_detectionOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool object_detectionOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
