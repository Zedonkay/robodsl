...
detectionOnnxManager::detectionOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "detection_onnx");
}
...
bool detectionOnnxManager::initialize() {
    // Set model paths
    detection_model_model_path_ = "models/detection_model.onnx";
...
bool detectionOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for detection_model: " << e.what() << std::endl;
        return false;
    }
...
void detectionOnnxManager::cleanup() {
    sessions_.clear();
}

bool detectionOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load detection_model model
    try {
        auto session = std::make_unique<Ort::Session>(env_, detection_model_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load detection_model model: " << e.what() << std::endl;
        return false;
    }
...
bool detectionOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool detectionOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool detectionOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
