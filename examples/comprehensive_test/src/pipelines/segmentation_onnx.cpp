...
segmentationOnnxManager::segmentationOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "segmentation_onnx");
}
...
bool segmentationOnnxManager::initialize() {
    // Set model paths
    segmentation_model_model_path_ = "models/segmentation_model.onnx";
...
bool segmentationOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for segmentation_model: " << e.what() << std::endl;
        return false;
    }
...
void segmentationOnnxManager::cleanup() {
    sessions_.clear();
}

bool segmentationOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load segmentation_model model
    try {
        auto session = std::make_unique<Ort::Session>(env_, segmentation_model_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load segmentation_model model: " << e.what() << std::endl;
        return false;
    }
...
bool segmentationOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool segmentationOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool segmentationOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
