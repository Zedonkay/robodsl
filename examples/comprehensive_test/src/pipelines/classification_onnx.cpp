#include "robodsl_project/classification_onnx.hpp"
#include <iostream>
#include <fstream>

namespace robodsl_project {

classificationOnnxManager::classificationOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "classification_onnx");
}
...
bool classificationOnnxManager::initialize() {
    // Set model paths
    classification_model_model_path_ = "models/classification_model.onnx";
...
bool classificationOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    // Run classification_model model
    try {
        ...
        std::cerr << "ONNX inference failed for classification_model: " << e.what() << std::endl;
        return false;
    }
...
void classificationOnnxManager::cleanup() {
    sessions_.clear();
}

bool classificationOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load classification_model model
    try {
        auto session = std::make_unique<Ort::Session>(env_, classification_model_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load classification_model model: " << e.what() << std::endl;
        return false;
    }
...
bool classificationOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool classificationOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool classificationOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
