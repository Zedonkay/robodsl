...
: rclcpp_lifecycle::LifecycleNode("main_node", options)
{
    ...
    this->declare_parameter<list>("array_coords", [1.0, 2.0, 3.0]);
    this->declare_parameter<dict>("dict_config", {'max_iterations': 100, 'timeout': 5.0, 'enabled': True});

    // Create publishers
    {
        rclcpp::QoS qos(10);
        qos.reliability(rclcpp::reliable);
        chatter_pub_ = this->create_publisher<std_msgs::msg::String>(
            "/chatter", qos);
    }
    {
        rclcpp::QoS qos(5);
        qos.reliability(rclcpp::best_effort);
    ...
    // Create services with proper error handling
    {
        try {
            ...
            process_image_srv_ = this->create_service<ImageProcessing>(
                "/process_image", 
                callback,
    ...
    try {
        ...
        }
        {
            ...
            if (this->get_parameter("rate", param)) {
                ...
                // Store the parameter value
                rate_ = param.template get<double>();
            } else {
                RCLCPP_WARN(
                    this->get_logger(), 
                    "Failed to load parameter '%s', using default value", 
        ...
        }
        {
            ...
            if (this->get_parameter("name", param)) {
                ...
                // Store the parameter value
                name_ = param.template get<string>();
            } else {
                RCLCPP_WARN(
                    this->get_logger(), 
                    "Failed to load parameter '%s', using default value", 
        ...
        }
        {
            ...
            if (this->get_parameter("enabled", param)) {
                ...
                // Store the parameter value
                enabled_ = param.template get<bool>();
            } else {
                RCLCPP_WARN(
                    this->get_logger(), 
                    "Failed to load parameter '%s', using default value", 
        ...
        }
        {
            ...
            if (this->get_parameter("array_coords", param)) {
                ...
                // Store the parameter value
                array_coords_ = param.template get<list>();
            } else {
                RCLCPP_WARN(
                    this->get_logger(), 
                    "Failed to load parameter '%s', using default value", 
        ...
        }
        {
            ...
            if (this->get_parameter("dict_config", param)) {
                ...
                // Store the parameter value
                dict_config_ = param.template get<dict>();
            } else {
                RCLCPP_WARN(
                    this->get_logger(), 
                    "Failed to load parameter '%s', using default value", 
...
Main_nodeNode::on_activate(const rclcpp_lifecycle::State& previous_state)
{
    ...
    try {
        ...
        if (detections_pub_) {
            try {
                auto pub = std::dynamic_pointer_cast<rclcpp_lifecycle::LifecyclePublisher<DetectionResult>>(detections_pub_);
                if (pub) {
                    pub->on_activate();
                    RCLCPP_DEBUG(
                        this->get_logger(), 
...
void code) {
    if (enable_processing_) CPP_BLOCK_PLACEHOLDER
