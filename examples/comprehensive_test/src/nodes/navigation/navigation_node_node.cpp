// Generated by RoboDSL - DO NOT EDIT

#include "../include/navigation_node_node.hpp"
#include <memory>

// Global C++ code blocks (passed through as-is)

    // Additional C++ code that gets included in the generated files
    namespace robot_utils {
        // Utility functions
        template<typename T>
        T clamp(T value, T min, T max) {
            return std::max(min, std::min(max, value));
        }
        
        double radians_to_degrees(double radians) {
            return radians * 180.0 / M_PI;
        }
        
        double degrees_to_radians(double degrees) {
            return degrees * M_PI / 180.0;
        }
    }


namespace robodsl {

Navigation_nodeNode::Navigation_nodeNode(const rclcpp::NodeOptions& options)
: rclcpp::Node("navigation_node", options)
{
    // Initialize parameters
    this->declare_parameter<float>("max_velocity", 2.0);
    this->declare_parameter<float>("goal_tolerance", 0.1);
    this->declare_parameter<bool>("enable_obstacle_avoidance", True);

    // Create publishers
    {
        rclcpp::QoS qos(10);
        cmd_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(
            "/cmd_vel", qos);
    }
    {
        rclcpp::QoS qos(10);
        path_pub_ = this->create_publisher<nav_msgs::msg::Path>(
            "/path", qos);
    }

    // Create subscribers
    {
        rclcpp::QoS qos(10);
        auto callback = std::bind(
            &Navigation_nodeNode::on_goal_pose, 
            this, 
            std::placeholders::_1
        );
        goal_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>(
            "/goal_pose", 
            qos, 
            callback
        );
    }

    // Create services with proper error handling
    
    // Initialize action servers
    {
        using namespace std::placeholders;
        
        try {
            auto handle_goal = [this](
                const rclcpp_action::GoalUUID& uuid,
                std::shared_ptr<const NavigationAction::Goal> goal
            ) -> rclcpp_action::GoalResponse {
                RCLCPP_INFO(this->get_logger(), 
                    "Received goal request for action /navigate_to_pose");
                (void)uuid;
                (void)goal;
                return rclcpp_action::GoalResponse::ACCEPT_AND_EXECUTE;
            };
            
            auto handle_cancel = [this](
                const std::shared_ptr<rclcpp_action::ServerGoalHandle<NavigationAction>> goal_handle
            ) -> rclcpp_action::CancelResponse {
                RCLCPP_INFO(this->get_logger(), 
                    "Received request to cancel goal for action /navigate_to_pose");
                (void)goal_handle;
                return rclcpp_action::CancelResponse::ACCEPT;
            };
            
            auto handle_accepted = [this](
                const std::shared_ptr<rclcpp_action::ServerGoalHandle<NavigationAction>> goal_handle
            ) {
                std::thread{
                    std::bind(&Navigation_nodeNode::execute_/navigate_to_pose, this, _1), 
                    goal_handle
                }.detach();
            };
            
            /navigate_to_pose_action_server_ = rclcpp_action::create_server<NavigationAction>(
                this->get_node_base_interface(),
                this->get_node_clock_interface(),
                this->get_node_logging_interface(),
                this->get_node_waitables_interface(),
                "/navigate_to_pose",
                handle_goal,
                handle_cancel,
                handle_accepted
            );
            
            RCLCPP_INFO(this->get_logger(), 
                "Created action server: %s", "/navigate_to_pose");
                
        } catch (const std::exception& e) {
            RCLCPP_ERROR(this->get_logger(), 
                "Failed to create action server /navigate_to_pose: %s", e.what());
            throw;
        }
    }
}

Navigation_nodeNode::~Navigation_nodeNode()
{
    // Clean up CUDA resources if enabled
    
    // Cleanup CUDA resources if needed
}

// Lifecycle callbacks
void
Navigation_nodeNode::on_configure(const rclcpp_lifecycle::State& previous_state)
{
    (void)previous_state;
}

void
Navigation_nodeNode::on_activate(const rclcpp_lifecycle::State& previous_state)
{
    (void)previous_state;
}

void
Navigation_nodeNode::on_deactivate(const rclcpp_lifecycle::State& previous_state)
{
    (void)previous_state;
}

void
Navigation_nodeNode::on_cleanup(const rclcpp_lifecycle::State& previous_state)
{
    (void)previous_state;
}

void
Navigation_nodeNode::on_shutdown(const rclcpp_lifecycle::State& previous_state)
{
    (void)previous_state;
}

// Timer callbacks
void Navigation_nodeNode::on_navigation_timer()
{
    // Timer callback implementation
    RCLCPP_DEBUG(this->get_logger(), "Timer navigation_timer triggered");
}


// Subscriber callbacks
void Navigation_nodeNode::on_goal_pose(const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg)
{
    // Process message
    RCLCPP_DEBUG(this->get_logger(), "Received message on /goal_pose");
}


// Service callbacks

// CUDA methods

// User-defined C++ methods
void Navigation_nodeNode::navigation_step(
void code) {
    process_navigation_step();
        
}

// Raw C++ code blocks (passed through as-is)

} // namespace robodsl
