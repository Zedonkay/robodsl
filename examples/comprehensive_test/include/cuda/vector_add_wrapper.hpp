...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for vector_add
 * 
 ...
 */
class vector_addWrapper {
public:
    /**
     * @brief Construct a new vector_add Wrapper object
     * 
     ...
     */
    explicit vector_addWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating vector_addWrapper");
        }
    }
    
    /**
     * @brief Destroy the vector_add Wrapper object
     * 
    ...
    ~vector_addWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying vector_addWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    vector_addWrapper(const vector_addWrapper&) = delete;
    vector_addWrapper& operator=(const vector_addWrapper&) = delete;
    
    // Allow move constructor and assignment
    vector_addWrapper(vector_addWrapper&& other) noexcept
    ...
    vector_addWrapper& operator=(vector_addWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized vector_addWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            ...
            cudaError_t status = cudaMalloc(&d_size_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_size_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_size_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_size_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_size_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_size_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        vector_add_kernel<<<grid_size, block_size, 1024, stream_>>>(
            d_a_,            d_b_,            d_size_,            d_c_            num_elements
        );
...
inline std::unique_ptr<vector_addWrapper> createvector_addWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<vector_addWrapper>(node, stream);
}
