...
// CUDA kernel header
#include "cuda/matrix_multiply_kernel.cuh"

namespace robodsl {

/**
 * @brief ROS2-compatible CUDA kernel wrapper for matrix_multiply
 * 
 ...
 */
class matrix_multiplyWrapper {
public:
    /**
     * @brief Construct a new matrix_multiply Wrapper object
     * 
     ...
     */
    explicit matrix_multiplyWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating matrix_multiplyWrapper");
        }
    }
    
    /**
     * @brief Destroy the matrix_multiply Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~matrix_multiplyWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying matrix_multiplyWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    matrix_multiplyWrapper(const matrix_multiplyWrapper&) = delete;
    matrix_multiplyWrapper& operator=(const matrix_multiplyWrapper&) = delete;
    
    // Allow move constructor and assignment
    matrix_multiplyWrapper(matrix_multiplyWrapper&& other) noexcept
    ...
    matrix_multiplyWrapper& operator=(matrix_multiplyWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized matrix_multiplyWrapper on device %d: %s", 
...
private:
    ...
    int* d_m_ = nullptr;  //!< Device memory for m
    int* d_n_ = nullptr;  //!< Device memory for n
    int* d_k_ = nullptr;  //!< Device memory for k
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_a_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_a_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_b_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_b_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_m_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_m_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_n_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_n_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_k_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_k_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_c_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_c_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_a_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_a_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_b_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_b_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_m_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_m_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_n_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_n_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_k_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_k_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_c_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_c_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_a_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_a_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_b_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_b_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_m_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_m_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_n_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_n_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_k_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_k_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_c_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_c_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        matrix_multiply_kernel<<<grid_size, block_size, 2048, stream_>>>(
            d_a_,            d_b_,            d_m_,            d_n_,            d_k_,            d_c_            num_elements
        );
...
inline std::unique_ptr<matrix_multiplyWrapper> creatematrix_multiplyWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<matrix_multiplyWrapper>(node, stream);
}
