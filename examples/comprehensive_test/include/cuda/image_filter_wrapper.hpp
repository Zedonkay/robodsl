...
// CUDA kernel header
#include "cuda/image_filter_kernel.cuh"

namespace robodsl {

/**
 * @brief ROS2-compatible CUDA kernel wrapper for image_filter
 * 
 ...
 */
class image_filterWrapper {
public:
    /**
     * @brief Construct a new image_filter Wrapper object
     * 
     ...
     */
    explicit image_filterWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating image_filterWrapper");
        }
    }
    
    /**
     * @brief Destroy the image_filter Wrapper object
     * 
    ...
    ~image_filterWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying image_filterWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    image_filterWrapper(const image_filterWrapper&) = delete;
    image_filterWrapper& operator=(const image_filterWrapper&) = delete;
    
    // Allow move constructor and assignment
    image_filterWrapper(image_filterWrapper&& other) noexcept
    ...
    image_filterWrapper& operator=(image_filterWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized image_filterWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_input_image_, input_size * sizeof(uchar*));
            ...
            cudaError_t status = cudaMalloc(&d_output_image_, input_size * sizeof(uchar*));
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        image_filter_kernel<<<grid_size, block_size, 4096, stream_>>>(
            d_input_image_,            d_width_,            d_height_,            d_output_image_            num_elements
        );
    ...
    bool launchKernelWithParameters(size_t num_elements, const uchar*& parameters) {
        auto start_time = std::chrono::high_resolution_clock::now();
...
inline std::unique_ptr<image_filterWrapper> createimage_filterWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<image_filterWrapper>(node, stream);
}
