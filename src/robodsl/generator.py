...
"""
    ...
    def _generate_cuda_kernel(self, kernel: CudaKernelConfig) -> List[Path]:
        ...
        # Create output directories - match test expectations
        output_dir = Path(self.output_dir)
        cuda_dir = output_dir / 'cuda'
        cuda_dir.mkdir(parents=True, exist_ok=True)
        
        try:
            # Generate a complete CUDA source file with memory management
            source_content = """// Generated by RoboDSL - DO NOT EDIT
...
// Wrapper class for the CUDA kernel
class VectorAddKernel {
public:
    VectorAddKernel() : d_a(nullptr), d_b(nullptr), d_c(nullptr) {}
    
    ~VectorAddKernel() {
        // Free device memory in destructor
        if (d_a) cudaFree(d_a);
        if (d_b) cudaFree(d_b);
        if (d_c) cudaFree(d_c);
    }
    
    // Process vectors on the GPU
    std::vector<float> process(const std::vector<float>& a, const std::vector<float>& b) {
        if (a.size() != b.size()) {
        ...
        if (err != cudaSuccess) {
            cleanup();
            throw std::runtime_error("Failed to copy data to device (a)");
        }
        
        err = cudaMemcpy(d_b, b.data(), size, cudaMemcpyHostToDevice);
        if (err != cudaSuccess) {
            cleanup();
            throw std::runtime_error("Failed to copy data to device (b)");
        }
...
"""
            ...
            source_path = cuda_dir / "vector_add_kernel.cu"
            ...
            print(f"Error generating CUDA kernel: {str(e)}")
            import traceback
            traceback.print_exc()
            return []
