...
'''
    ...
    def _generate_cmakelists(self) -> Path:
        ...
        """
        # Check if any nodes use ROS2 features
        uses_ros2 = any(hasattr(node, 'uses_ros2') and node.uses_ros2 
                       for node in self.config.nodes)
        
        # Check for specific ROS2 features
        ...
        has_qos_settings = any(
            getattr(node, 'qos', None) is not None
            for node in self.config.nodes
        )
        
        has_cuda_kernels = any(
            hasattr(node, 'cuda_kernels') and node.cuda_kernels
            for node in self.config.nodes
        )
        ...
        cmake_content = [
            ...
            f"project({self.config.project_name} VERSION 0.1.0 LANGUAGES CXX)",
            "",
            "# Default to C++17",
            ...
            "enable_testing()",
            "",
            "# Find Python for Python modules",
            "find_package(Python COMPONENTS Interpreter Development REQUIRED)",
            "",
            "# Get Python version for installation paths",
            "execute_process(",
            ...
            "",
            "# Build options",
            f"option(ENABLE_CUDA \"Enable CUDA support\" {'ON' if has_cuda_kernels else 'OFF'})",
            f"option(ENABLE_ROS2 \"Enable ROS2 support\" {'ON' if uses_ros2 else 'OFF'})",
            ""
        ]
        
        # Add CUDA configuration if needed
        if has_cuda_kernels:
            cmake_content.extend([
                ...
                "    endif()",
                "else()",
                "    add_compile_definitions(WITH_CUDA=0 WITH_THRUST=0)",
                "endif()",
                ""
            ])
        
        # Add ROS2 configuration if needed
        if uses_ros2:
            cmake_content.extend([
                "# ROS2 Configuration",
                "if(ENABLE_ROS2)",
            ...
            # Export dependencies
            deps = ["rclcpp", "rclcpp_components", "std_msgs"]
            if has_lifecycle_nodes:
                deps.extend(["rclcpp_lifecycle", "lifecycle_msgs"])
            if has_qos_settings:
                deps.extend(["rcl", "rmw_implementation"])
                
            cmake_content.append("    # Export dependencies")
            cmake_content.append(f"    ament_export_dependencies({' '.join(deps)})")
        ...
        if has_cuda_kernels:
            cmake_content.append("    ${CUDA_LIBRARIES}")
        
        # Add ROS2 dependencies if enabled
        if uses_ros2:
            cmake_content.append("    ${rclcpp_LIBRARIES}")
            if has_lifecycle_nodes:
                cmake_content.append("    ${rclcpp_lifecycle_LIBRARIES}")
        ...
        for node in self.config.nodes:
            ...
            cmake_content.extend([
                ...
                f"    src/nodes/{node_name}_node.cpp",
                ")",
                "",
            ...
            if hasattr(node, 'uses_ros2') and node.uses_ros2:
                cmake_content.extend([
                    f"    # Register {node_name} as a component",
                    f"    rclcpp_components_register_nodes({node_name}_node \"{self.config.project_name}::{node_name}Component\")",
                    "",
