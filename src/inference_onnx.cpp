#include "robodsl_project/inference_onnx.hpp"
#include <iostream>
#include <fstream>

namespace robodsl_project {

inferenceOnnxManager::inferenceOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "inference_onnx");
}
...
bool inferenceOnnxManager::initialize() {
    // Set model paths
    pipeline_model_model_path_ = "models/pipeline_model.onnx";
...
bool inferenceOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for pipeline_model: " << e.what() << std::endl;
        return false;
    }
...
void inferenceOnnxManager::cleanup() {
    sessions_.clear();
}

bool inferenceOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load pipeline_model model
    try {
        auto session = std::make_unique<Ort::Session>(env_, pipeline_model_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load pipeline_model model: " << e.what() << std::endl;
        return false;
    }
...
bool inferenceOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool inferenceOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool inferenceOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
