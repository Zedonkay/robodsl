...
#ifndef INVERSE_KINEMATICS_KERNEL_HPP
#define INVERSE_KINEMATICS_KERNEL_HPP
...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for inverse_kinematics
 * 
 ...
 */
class inverse_kinematicsWrapper {
public:
    /**
     * @brief Construct a new inverse_kinematics Wrapper object
     * 
     ...
     */
    explicit inverse_kinematicsWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating inverse_kinematicsWrapper");
        }
    }
    
    /**
     * @brief Destroy the inverse_kinematics Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~inverse_kinematicsWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying inverse_kinematicsWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    inverse_kinematicsWrapper(const inverse_kinematicsWrapper&) = delete;
    inverse_kinematicsWrapper& operator=(const inverse_kinematicsWrapper&) = delete;
    
    // Allow move constructor and assignment
    inverse_kinematicsWrapper(inverse_kinematicsWrapper&& other) noexcept
        ...
        d_current_joints_ = other.d_current_joints_;
        other.d_current_joints_ = nullptr;
        d_joint_limits_ = other.d_joint_limits_;
        other.d_joint_limits_ = nullptr;
        d_computed_joints_ = other.d_computed_joints_;
        other.d_computed_joints_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    inverse_kinematicsWrapper& operator=(inverse_kinematicsWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized inverse_kinematicsWrapper on device %d: %s", 
...
private:
    ...
    float** d_current_joints_ = nullptr;  //!< Device memory for current_joints
    float** d_joint_limits_ = nullptr;  //!< Device memory for joint_limits
    float** d_computed_joints_ = nullptr;  //!< Device memory for computed_joints
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            ...
            cudaError_t status = cudaMalloc(&d_current_joints_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_current_joints_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_joint_limits_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_joint_limits_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_computed_joints_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_computed_joints_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_current_joints_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_current_joints_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_joint_limits_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_joint_limits_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_computed_joints_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_computed_joints_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_current_joints_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_current_joints_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_joint_limits_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_joint_limits_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_computed_joints_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_computed_joints_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        inverse_kinematics_kernel<<<grid_size, block_size, 1024, stream_>>>(
            d_target_pose_,            d_current_joints_,            d_joint_limits_,            d_computed_joints_            num_elements
        );
...
inline std::unique_ptr<inverse_kinematicsWrapper> createinverse_kinematicsWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<inverse_kinematicsWrapper>(node, stream);
}

} // namespace robodsl

#endif // INVERSE_KINEMATICS_KERNEL_HPP
