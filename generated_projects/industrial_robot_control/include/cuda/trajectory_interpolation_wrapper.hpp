...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for trajectory_interpolation
 * 
 ...
 */
class trajectory_interpolationWrapper {
public:
    /**
     * @brief Construct a new trajectory_interpolation Wrapper object
     * 
     ...
     */
    explicit trajectory_interpolationWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating trajectory_interpolationWrapper");
        }
    }
    
    /**
     * @brief Destroy the trajectory_interpolation Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~trajectory_interpolationWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying trajectory_interpolationWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    trajectory_interpolationWrapper(const trajectory_interpolationWrapper&) = delete;
    trajectory_interpolationWrapper& operator=(const trajectory_interpolationWrapper&) = delete;
    
    // Allow move constructor and assignment
    trajectory_interpolationWrapper(trajectory_interpolationWrapper&& other) noexcept
        ...
        d_waypoints_ = other.d_waypoints_;
        other.d_waypoints_ = nullptr;
        d_num_waypoints_ = other.d_num_waypoints_;
        other.d_num_waypoints_ = nullptr;
        d_time_stamps_ = other.d_time_stamps_;
        other.d_time_stamps_ = nullptr;
        d_trajectory_ = other.d_trajectory_;
        other.d_trajectory_ = nullptr;
        d_trajectory_length_ = other.d_trajectory_length_;
        other.d_trajectory_length_ = nullptr;
        d_interpolated_trajectory_ = other.d_interpolated_trajectory_;
        other.d_interpolated_trajectory_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    trajectory_interpolationWrapper& operator=(trajectory_interpolationWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized trajectory_interpolationWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_waypoints_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_waypoints_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_num_waypoints_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_num_waypoints_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_time_stamps_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_time_stamps_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_trajectory_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_trajectory_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_trajectory_length_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_trajectory_length_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_interpolated_trajectory_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_interpolated_trajectory_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_waypoints_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_waypoints_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_num_waypoints_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_num_waypoints_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_time_stamps_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_time_stamps_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_trajectory_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_trajectory_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_trajectory_length_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_trajectory_length_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_interpolated_trajectory_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_interpolated_trajectory_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_waypoints_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_waypoints_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_num_waypoints_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_num_waypoints_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_time_stamps_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_time_stamps_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_trajectory_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_trajectory_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_trajectory_length_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_trajectory_length_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_interpolated_trajectory_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_interpolated_trajectory_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        trajectory_interpolation_kernel<<<grid_size, block_size, 2048, stream_>>>(
            d_waypoints_,            d_num_waypoints_,            d_time_stamps_,            d_trajectory_,            d_trajectory_length_,            d_interpolated_trajectory_            num_elements
        );
...
inline std::unique_ptr<trajectory_interpolationWrapper> createtrajectory_interpolationWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<trajectory_interpolationWrapper>(node, stream);
}
