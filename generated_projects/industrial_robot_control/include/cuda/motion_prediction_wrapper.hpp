...
#ifndef MOTION_PREDICTION_KERNEL_HPP
#define MOTION_PREDICTION_KERNEL_HPP
...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for motion_prediction
 * 
 ...
 */
class motion_predictionWrapper {
public:
    /**
     * @brief Construct a new motion_prediction Wrapper object
     * 
     ...
     */
    explicit motion_predictionWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating motion_predictionWrapper");
        }
    }
    
    /**
     * @brief Destroy the motion_prediction Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~motion_predictionWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying motion_predictionWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    motion_predictionWrapper(const motion_predictionWrapper&) = delete;
    motion_predictionWrapper& operator=(const motion_predictionWrapper&) = delete;
    
    // Allow move constructor and assignment
    motion_predictionWrapper(motion_predictionWrapper&& other) noexcept
        ...
        d_joint_history_ = other.d_joint_history_;
        other.d_joint_history_ = nullptr;
        d_history_length_ = other.d_history_length_;
        other.d_history_length_ = nullptr;
        d_prediction_horizon_ = other.d_prediction_horizon_;
        other.d_prediction_horizon_ = nullptr;
        d_predicted_joints_ = other.d_predicted_joints_;
        other.d_predicted_joints_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    motion_predictionWrapper& operator=(motion_predictionWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized motion_predictionWrapper on device %d: %s", 
...
private:
    ...
    float** d_joint_history_ = nullptr;  //!< Device memory for joint_history
    int* d_history_length_ = nullptr;  //!< Device memory for history_length
    float** d_prediction_horizon_ = nullptr;  //!< Device memory for prediction_horizon
    float** d_predicted_joints_ = nullptr;  //!< Device memory for predicted_joints
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_joint_history_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_joint_history_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_history_length_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_history_length_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_prediction_horizon_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_prediction_horizon_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_predicted_joints_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_predicted_joints_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_joint_history_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_joint_history_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_history_length_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_history_length_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_prediction_horizon_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_prediction_horizon_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_predicted_joints_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_predicted_joints_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_joint_history_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_joint_history_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_history_length_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_history_length_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_prediction_horizon_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_prediction_horizon_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_predicted_joints_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_predicted_joints_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        motion_prediction_kernel<<<grid_size, block_size, 2048, stream_>>>(
            d_joint_history_,            d_history_length_,            d_prediction_horizon_,            d_predicted_joints_            num_elements
        );
...
inline std::unique_ptr<motion_predictionWrapper> createmotion_predictionWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<motion_predictionWrapper>(node, stream);
}

} // namespace robodsl

#endif // MOTION_PREDICTION_KERNEL_HPP
