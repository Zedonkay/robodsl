...
#ifndef FORCE_CONTROL_KERNEL_HPP
#define FORCE_CONTROL_KERNEL_HPP
...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for force_control
 * 
 ...
 */
class force_controlWrapper {
public:
    /**
     * @brief Construct a new force_control Wrapper object
     * 
     ...
     */
    explicit force_controlWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating force_controlWrapper");
        }
    }
    
    /**
     * @brief Destroy the force_control Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~force_controlWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying force_controlWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    force_controlWrapper(const force_controlWrapper&) = delete;
    force_controlWrapper& operator=(const force_controlWrapper&) = delete;
    
    // Allow move constructor and assignment
    force_controlWrapper(force_controlWrapper&& other) noexcept
        ...
        d_desired_force_ = other.d_desired_force_;
        other.d_desired_force_ = nullptr;
        d_measured_force_ = other.d_measured_force_;
        other.d_measured_force_ = nullptr;
        d_current_pose_ = other.d_current_pose_;
        other.d_current_pose_ = nullptr;
        d_stiffness_matrix_ = other.d_stiffness_matrix_;
        other.d_stiffness_matrix_ = nullptr;
        d_force_correction_ = other.d_force_correction_;
        other.d_force_correction_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    force_controlWrapper& operator=(force_controlWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized force_controlWrapper on device %d: %s", 
...
private:
    ...
    float** d_desired_force_ = nullptr;  //!< Device memory for desired_force
    float** d_measured_force_ = nullptr;  //!< Device memory for measured_force
    float** d_current_pose_ = nullptr;  //!< Device memory for current_pose
    float** d_stiffness_matrix_ = nullptr;  //!< Device memory for stiffness_matrix
    float** d_force_correction_ = nullptr;  //!< Device memory for force_correction
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_desired_force_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_desired_force_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_measured_force_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_measured_force_")) {
                return false;
            }
            ...
            cudaError_t status = cudaMalloc(&d_stiffness_matrix_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_stiffness_matrix_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_force_correction_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_force_correction_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_desired_force_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_desired_force_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_measured_force_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_measured_force_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(d_stiffness_matrix_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_stiffness_matrix_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_force_correction_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_force_correction_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_desired_force_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_desired_force_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_measured_force_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_measured_force_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_stiffness_matrix_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_stiffness_matrix_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_force_correction_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_force_correction_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        force_control_kernel<<<grid_size, block_size, 1024, stream_>>>(
            d_desired_force_,            d_measured_force_,            d_current_pose_,            d_stiffness_matrix_,            d_force_correction_            num_elements
        );
...
inline std::unique_ptr<force_controlWrapper> createforce_controlWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<force_controlWrapper>(node, stream);
}

} // namespace robodsl

#endif // FORCE_CONTROL_KERNEL_HPP
