#include "collision_predictor_onnx.hpp"
#include <iostream>
#include <fstream>
#include <stdexcept>
#include <algorithm>
...
collision_predictorOnnxInference::collision_predictorOnnxInference(const std::string& model_path)
    : model_path_(model_path), device_type_("gpu") {
    
    // Set up environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "collision_predictor_onnx");
...
collision_predictorOnnxInference::~collision_predictorOnnxInference() {
    // Free CUDA memory
...
bool collision_predictorOnnxInference::initialize() {
    try {
...
bool collision_predictorOnnxInference::run_inference(const std::vector<float>& input_data, std::vector<float>& output_data) {
    try {
...
bool collision_predictorOnnxInference::run_inference_cuda(const float* input_data, size_t input_size, 
...
void* collision_predictorOnnxInference::allocate_cuda_memory(size_t size) {
    void* ptr = nullptr;
...
void collision_predictorOnnxInference::free_cuda_memory(void* ptr) {
    if (ptr) {
        cudaFree(ptr);
...
bool collision_predictorOnnxInference::copy_to_cuda(const std::vector<float>& host_data, void* cuda_ptr) {
    cudaError_t error = cudaMemcpy(cuda_ptr, host_data.data(), 
                                   host_data.size() * sizeof(float), cudaMemcpyHostToDevice);
    return error == cudaSuccess;
}

bool collision_predictorOnnxInference::copy_from_cuda(void* cuda_ptr, std::vector<float>& host_data) {
    cudaError_t error = cudaMemcpy(host_data.data(), cuda_ptr, 
                                   host_data.size() * sizeof(float), cudaMemcpyDeviceToHost);
    return error == cudaSuccess;
}

void collision_predictorOnnxInference::initialize_session_options(Ort::SessionOptions& session_options) {
...
std::vector<float> collision_predictorOnnxInference::preprocess_input(const std::vector<float>& raw_input) {
...
std::vector<float> collision_predictorOnnxInference::postprocess_output(const std::vector<float>& raw_output) {
...
bool collision_predictorOnnxInference::run_inference_tensorrt(const std::vector<float>& input_data, std::vector<float>& output_data) {
...
bool collision_predictorOnnxInference::run_inference_tensorrt_cuda(const float* input_data, size_t input_size, 
                                                              float* output_data, size_t output_size) {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
...
bool collision_predictorOnnxInference::enable_tensorrt_fp16() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool collision_predictorOnnxInference::enable_tensorrt_int8() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool collision_predictorOnnxInference::set_tensorrt_workspace_size(size_t size) {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool collision_predictorOnnxInference::clear_tensorrt_cache() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool collision_predictorOnnxInference::initialize_tensorrt_options(OrtTensorRTProviderOptions& trt_options) {
    trt_options.device_id = 0;
...
bool collision_predictorOnnxInference::create_tensorrt_cache_directory() {
    std::string command = "mkdir -p " + tensorrt_cache_path_;
    int result = system(command.c_str());
    return result == 0;
}

bool collision_predictorOnnxInference::validate_tensorrt_engine() {
    if (!tensorrt_enabled_) {
        return false;
    }
