#include "robodsl_project/force_estimation_onnx.hpp"
#include <iostream>
#include <fstream>

namespace robodsl_project {

force_estimationOnnxManager::force_estimationOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "force_estimation_onnx");
}
...
bool force_estimationOnnxManager::initialize() {
    // Set model paths
    force_estimator_model_path_ = "models/force_estimator.onnx";
...
bool force_estimationOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for force_estimator: " << e.what() << std::endl;
        return false;
    }
...
void force_estimationOnnxManager::cleanup() {
    sessions_.clear();
}

bool force_estimationOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load force_estimator model
    try {
        auto session = std::make_unique<Ort::Session>(env_, force_estimator_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load force_estimator model: " << e.what() << std::endl;
        return false;
    }
...
bool force_estimationOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool force_estimationOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool force_estimationOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
