...
motion_optimizationOnnxManager::motion_optimizationOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "motion_optimization_onnx");
}
...
bool motion_optimizationOnnxManager::initialize() {
    // Set model paths
    trajectory_optimizer_model_path_ = "models/trajectory_optimizer.onnx";
...
bool motion_optimizationOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    // Run trajectory_optimizer model
    try {
        ...
        std::cerr << "ONNX inference failed for trajectory_optimizer: " << e.what() << std::endl;
        return false;
    }
...
void motion_optimizationOnnxManager::cleanup() {
    sessions_.clear();
}

bool motion_optimizationOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load trajectory_optimizer model
    try {
        auto session = std::make_unique<Ort::Session>(env_, trajectory_optimizer_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load trajectory_optimizer model: " << e.what() << std::endl;
        return false;
    }
...
bool motion_optimizationOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool motion_optimizationOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool motion_optimizationOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
