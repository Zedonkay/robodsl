...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for trajectory_planning
 * 
 ...
 */
class trajectory_planningWrapper {
public:
    /**
     * @brief Construct a new trajectory_planning Wrapper object
     * 
     ...
     */
    explicit trajectory_planningWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating trajectory_planningWrapper");
        }
    }
    
    /**
     * @brief Destroy the trajectory_planning Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~trajectory_planningWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying trajectory_planningWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    trajectory_planningWrapper(const trajectory_planningWrapper&) = delete;
    trajectory_planningWrapper& operator=(const trajectory_planningWrapper&) = delete;
    
    // Allow move constructor and assignment
    trajectory_planningWrapper(trajectory_planningWrapper&& other) noexcept
        ...
        d_current_pose_ = other.d_current_pose_;
        other.d_current_pose_ = nullptr;
        d_target_pose_ = other.d_target_pose_;
        other.d_target_pose_ = nullptr;
        ...
        d_planned_trajectory_ = other.d_planned_trajectory_;
        other.d_planned_trajectory_ = nullptr;
    ...
    trajectory_planningWrapper& operator=(trajectory_planningWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized trajectory_planningWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_current_pose_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_current_pose_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_target_pose_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_target_pose_")) {
                return false;
            }
            ...
            cudaError_t status = cudaMalloc(&d_planned_trajectory_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_planned_trajectory_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_current_pose_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_current_pose_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_target_pose_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_target_pose_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(d_planned_trajectory_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_planned_trajectory_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_current_pose_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_current_pose_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_target_pose_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_target_pose_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_planned_trajectory_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_planned_trajectory_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        trajectory_planning_kernel<<<grid_size, block_size, 4096, stream_>>>(
            d_current_pose_,            d_target_pose_,            d_obstacles_,            d_num_obstacles_,            d_planned_trajectory_,            d_trajectory_length_            num_elements
        );
...
inline std::unique_ptr<trajectory_planningWrapper> createtrajectory_planningWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<trajectory_planningWrapper>(node, stream);
}
