...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for safety_monitoring
 * 
 ...
 */
class safety_monitoringWrapper {
public:
    /**
     * @brief Construct a new safety_monitoring Wrapper object
     * 
     ...
     */
    explicit safety_monitoringWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating safety_monitoringWrapper");
        }
    }
    
    /**
     * @brief Destroy the safety_monitoring Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~safety_monitoringWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying safety_monitoringWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    safety_monitoringWrapper(const safety_monitoringWrapper&) = delete;
    safety_monitoringWrapper& operator=(const safety_monitoringWrapper&) = delete;
    
    // Allow move constructor and assignment
    safety_monitoringWrapper(safety_monitoringWrapper&& other) noexcept
        ...
        d_vehicle_state_ = other.d_vehicle_state_;
        other.d_vehicle_state_ = nullptr;
        d_obstacles_ = other.d_obstacles_;
        other.d_obstacles_ = nullptr;
        d_num_obstacles_ = other.d_num_obstacles_;
        other.d_num_obstacles_ = nullptr;
        d_safety_thresholds_ = other.d_safety_thresholds_;
        other.d_safety_thresholds_ = nullptr;
        d_safety_violations_ = other.d_safety_violations_;
        other.d_safety_violations_ = nullptr;
        d_risk_scores_ = other.d_risk_scores_;
        other.d_risk_scores_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    safety_monitoringWrapper& operator=(safety_monitoringWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized safety_monitoringWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_vehicle_state_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_vehicle_state_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_obstacles_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_obstacles_")) {
                return false;
            }
            ...
            cudaError_t status = cudaMalloc(&d_safety_thresholds_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_safety_thresholds_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_safety_violations_, input_size * sizeof(bool*));
            if (!checkCudaError(status, "cudaMalloc for d_safety_violations_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_risk_scores_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_risk_scores_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_vehicle_state_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_vehicle_state_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_obstacles_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_obstacles_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(d_safety_thresholds_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_safety_thresholds_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_safety_violations_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_safety_violations_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_risk_scores_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_risk_scores_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_vehicle_state_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_vehicle_state_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_obstacles_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_obstacles_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_safety_thresholds_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_safety_thresholds_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_safety_violations_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_safety_violations_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_risk_scores_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_risk_scores_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        safety_monitoring_kernel<<<grid_size, block_size, 1024, stream_>>>(
            d_vehicle_state_,            d_obstacles_,            d_num_obstacles_,            d_safety_thresholds_,            d_safety_violations_,            d_risk_scores_            num_elements
        );
...
inline std::unique_ptr<safety_monitoringWrapper> createsafety_monitoringWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<safety_monitoringWrapper>(node, stream);
}
