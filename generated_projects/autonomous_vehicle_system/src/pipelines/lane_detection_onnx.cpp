...
lane_detectionOnnxManager::lane_detectionOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "lane_detection_onnx");
}
...
bool lane_detectionOnnxManager::initialize() {
    // Set model paths
    lane_detector_model_path_ = "models/lane_detector.onnx";
...
bool lane_detectionOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for lane_detector: " << e.what() << std::endl;
        return false;
    }
...
void lane_detectionOnnxManager::cleanup() {
    sessions_.clear();
}

bool lane_detectionOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load lane_detector model
    try {
        auto session = std::make_unique<Ort::Session>(env_, lane_detector_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load lane_detector model: " << e.what() << std::endl;
        return false;
    }
...
bool lane_detectionOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool lane_detectionOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool lane_detectionOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
