...
behavior_predictionOnnxManager::behavior_predictionOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "behavior_prediction_onnx");
}
...
bool behavior_predictionOnnxManager::initialize() {
    // Set model paths
    behavior_predictor_model_path_ = "models/behavior_predictor.onnx";
...
bool behavior_predictionOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for behavior_predictor: " << e.what() << std::endl;
        return false;
    }
...
void behavior_predictionOnnxManager::cleanup() {
    sessions_.clear();
}

bool behavior_predictionOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load behavior_predictor model
    try {
        auto session = std::make_unique<Ort::Session>(env_, behavior_predictor_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load behavior_predictor model: " << e.what() << std::endl;
        return false;
    }
...
bool behavior_predictionOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool behavior_predictionOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool behavior_predictionOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
