...
traffic_sign_detectionOnnxManager::traffic_sign_detectionOnnxManager() {
    // Initialize ONNX Runtime environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "traffic_sign_detection_onnx");
}
...
bool traffic_sign_detectionOnnxManager::initialize() {
    // Set model paths
    traffic_sign_detector_model_path_ = "models/traffic_sign_detector.onnx";
...
bool traffic_sign_detectionOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    // Run traffic_sign_detector model
    try {
        ...
        std::cerr << "ONNX inference failed for traffic_sign_detector: " << e.what() << std::endl;
        return false;
    }
...
void traffic_sign_detectionOnnxManager::cleanup() {
    sessions_.clear();
}

bool traffic_sign_detectionOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load traffic_sign_detector model
    try {
        auto session = std::make_unique<Ort::Session>(env_, traffic_sign_detector_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load traffic_sign_detector model: " << e.what() << std::endl;
        return false;
    }
...
bool traffic_sign_detectionOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool traffic_sign_detectionOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool traffic_sign_detectionOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
