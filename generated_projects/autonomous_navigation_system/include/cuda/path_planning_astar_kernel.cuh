...
/**
 * @brief CUDA kernel wrapper class for path_planning_astar
 * 
 ...
 */
class path_planning_astarKernel {
public:
    /**
     * @brief Construct a new path_planning_astar Kernel object
     * 
     * @param stream CUDA stream to use for kernel execution (nullptr for default stream)
     */
    explicit path_planning_astarKernel(cudaStream_t* stream = nullptr);
    
    /**
     * @brief Destroy the path_planning_astar Kernel object
     * 
     * Frees all allocated device memory.
     */
    ~path_planning_astarKernel();
    
    // Delete copy constructor and assignment operator
    path_planning_astarKernel(const path_planning_astarKernel&) = delete;
    path_planning_astarKernel& operator=(const path_planning_astarKernel&) = delete;
     ...
     * @return std::vector<int*> Processed output data
     */
    // Process the input data using the CUDA kernel (host pointer and size)
    bool process(const int** parameters, size_t param_size, int** output, size_t output_size);
...
__global__ void path_planning_astar_kernel(    const float** d_occupancy_grid_,    const int* d_grid_width_,    const int* d_grid_height_,    const int* d_start_x_,    const int* d_start_y_,    const int* d_goal_x_,    const int* d_goal_y_,    const int** d_path_x_,    const int** d_path_y_,    const int** d_path_length_,    int num_elements
);
