...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for point_cloud_filter
 * 
 ...
 */
class point_cloud_filterWrapper {
public:
    /**
     * @brief Construct a new point_cloud_filter Wrapper object
     * 
     ...
     */
    explicit point_cloud_filterWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating point_cloud_filterWrapper");
        }
    }
    
    /**
     * @brief Destroy the point_cloud_filter Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~point_cloud_filterWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying point_cloud_filterWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    point_cloud_filterWrapper(const point_cloud_filterWrapper&) = delete;
    point_cloud_filterWrapper& operator=(const point_cloud_filterWrapper&) = delete;
    
    // Allow move constructor and assignment
    point_cloud_filterWrapper(point_cloud_filterWrapper&& other) noexcept
        ...
        d_raw_points_ = other.d_raw_points_;
        other.d_raw_points_ = nullptr;
        d_num_points_ = other.d_num_points_;
        other.d_num_points_ = nullptr;
        d_min_distance_ = other.d_min_distance_;
        other.d_min_distance_ = nullptr;
        d_max_distance_ = other.d_max_distance_;
        other.d_max_distance_ = nullptr;
        d_filtered_points_ = other.d_filtered_points_;
        other.d_filtered_points_ = nullptr;
        d_filtered_count_ = other.d_filtered_count_;
        other.d_filtered_count_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    point_cloud_filterWrapper& operator=(point_cloud_filterWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized point_cloud_filterWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_raw_points_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_raw_points_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_num_points_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_num_points_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_min_distance_, input_size * sizeof(float));
            if (!checkCudaError(status, "cudaMalloc for d_min_distance_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_max_distance_, input_size * sizeof(float));
            if (!checkCudaError(status, "cudaMalloc for d_max_distance_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_filtered_points_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_filtered_points_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_filtered_count_, input_size * sizeof(int*));
            if (!checkCudaError(status, "cudaMalloc for d_filtered_count_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_raw_points_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_raw_points_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_num_points_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_num_points_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_min_distance_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_min_distance_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_max_distance_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_max_distance_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_filtered_points_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_filtered_points_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_filtered_count_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_filtered_count_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_raw_points_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_raw_points_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_num_points_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_num_points_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_min_distance_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_min_distance_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_max_distance_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_max_distance_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_filtered_points_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_filtered_points_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_filtered_count_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_filtered_count_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        point_cloud_filter_kernel<<<grid_size, block_size, 1024, stream_>>>(
            d_raw_points_,            d_num_points_,            d_min_distance_,            d_max_distance_,            d_filtered_points_,            d_filtered_count_            num_elements
        );
...
inline std::unique_ptr<point_cloud_filterWrapper> createpoint_cloud_filterWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<point_cloud_filterWrapper>(node, stream);
}
