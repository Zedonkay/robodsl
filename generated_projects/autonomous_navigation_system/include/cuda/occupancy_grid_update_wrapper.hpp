...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for occupancy_grid_update
 * 
 ...
 */
class occupancy_grid_updateWrapper {
public:
    /**
     * @brief Construct a new occupancy_grid_update Wrapper object
     * 
     ...
     */
    explicit occupancy_grid_updateWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating occupancy_grid_updateWrapper");
        }
    }
    
    /**
     * @brief Destroy the occupancy_grid_update Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~occupancy_grid_updateWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying occupancy_grid_updateWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    occupancy_grid_updateWrapper(const occupancy_grid_updateWrapper&) = delete;
    occupancy_grid_updateWrapper& operator=(const occupancy_grid_updateWrapper&) = delete;
    
    // Allow move constructor and assignment
    occupancy_grid_updateWrapper(occupancy_grid_updateWrapper&& other) noexcept
        ...
        d_grid_data_ = other.d_grid_data_;
        other.d_grid_data_ = nullptr;
        ...
        d_resolution_ = other.d_resolution_;
        other.d_resolution_ = nullptr;
        d_robot_pose_ = other.d_robot_pose_;
        other.d_robot_pose_ = nullptr;
        d_updated_grid_ = other.d_updated_grid_;
        other.d_updated_grid_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    occupancy_grid_updateWrapper& operator=(occupancy_grid_updateWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized occupancy_grid_updateWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            ...
            cudaError_t status = cudaMalloc(&d_grid_data_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_grid_data_")) {
                return false;
            }
            ...
            cudaError_t status = cudaMalloc(&d_resolution_, input_size * sizeof(float));
            if (!checkCudaError(status, "cudaMalloc for d_resolution_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_robot_pose_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_robot_pose_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_updated_grid_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_updated_grid_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_grid_data_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_grid_data_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(d_resolution_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_resolution_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_robot_pose_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_robot_pose_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_updated_grid_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_updated_grid_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_grid_data_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_grid_data_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_resolution_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_resolution_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_robot_pose_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_robot_pose_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_updated_grid_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_updated_grid_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        occupancy_grid_update_kernel<<<grid_size, block_size, 2048, stream_>>>(
            d_filtered_points_,            d_num_points_,            d_grid_data_,            d_grid_width_,            d_grid_height_,            d_resolution_,            d_robot_pose_,            d_updated_grid_            num_elements
        );
...
inline std::unique_ptr<occupancy_grid_updateWrapper> createoccupancy_grid_updateWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<occupancy_grid_updateWrapper>(node, stream);
}
