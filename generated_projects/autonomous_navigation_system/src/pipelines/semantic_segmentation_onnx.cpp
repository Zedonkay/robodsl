...
semantic_segmentationOnnxManager::semantic_segmentationOnnxManager() {
...
bool semantic_segmentationOnnxManager::initialize() {
    // Set model paths
    semantic_segmentation_model_path_ = "models/semantic_segmentation.onnx";
...
bool semantic_segmentationOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    // Run semantic_segmentation model
    try {
        ...
        std::cerr << "ONNX inference failed for semantic_segmentation: " << e.what() << std::endl;
        return false;
    }
...
void semantic_segmentationOnnxManager::cleanup() {
    sessions_.clear();
}

bool semantic_segmentationOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load semantic_segmentation model
    try {
        auto session = std::make_unique<Ort::Session>(env_, semantic_segmentation_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load semantic_segmentation model: " << e.what() << std::endl;
        return false;
    }
...
bool semantic_segmentationOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool semantic_segmentationOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool semantic_segmentationOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
