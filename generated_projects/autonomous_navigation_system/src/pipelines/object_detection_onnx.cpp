...
bool object_detectionOnnxManager::initialize() {
    // Set model paths
    yolo_detector_model_path_ = "models/yolo_detector.onnx";
...
bool object_detectionOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for yolo_detector: " << e.what() << std::endl;
        return false;
    }
...
bool object_detectionOnnxManager::load_models() {
    ...
    // Load yolo_detector model
    try {
        auto session = std::make_unique<Ort::Session>(env_, yolo_detector_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load yolo_detector model: " << e.what() << std::endl;
        return false;
    }
