#include "semantic_segmentation_onnx.hpp"
#include <iostream>
#include <fstream>
#include <stdexcept>
#include <algorithm>
...
semantic_segmentationOnnxInference::semantic_segmentationOnnxInference(const std::string& model_path)
    : model_path_(model_path), device_type_("gpu") {
    
    // Set up environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "semantic_segmentation_onnx");
...
semantic_segmentationOnnxInference::~semantic_segmentationOnnxInference() {
    // Free CUDA memory
...
bool semantic_segmentationOnnxInference::initialize() {
    try {
...
bool semantic_segmentationOnnxInference::run_inference(const std::vector<float>& input_data, std::vector<float>& output_data) {
    try {
...
bool semantic_segmentationOnnxInference::run_inference_cuda(const float* input_data, size_t input_size, 
...
void* semantic_segmentationOnnxInference::allocate_cuda_memory(size_t size) {
    void* ptr = nullptr;
...
void semantic_segmentationOnnxInference::free_cuda_memory(void* ptr) {
    if (ptr) {
        cudaFree(ptr);
...
bool semantic_segmentationOnnxInference::copy_to_cuda(const std::vector<float>& host_data, void* cuda_ptr) {
    cudaError_t error = cudaMemcpy(cuda_ptr, host_data.data(), 
                                   host_data.size() * sizeof(float), cudaMemcpyHostToDevice);
    return error == cudaSuccess;
}

bool semantic_segmentationOnnxInference::copy_from_cuda(void* cuda_ptr, std::vector<float>& host_data) {
    cudaError_t error = cudaMemcpy(host_data.data(), cuda_ptr, 
                                   host_data.size() * sizeof(float), cudaMemcpyDeviceToHost);
    return error == cudaSuccess;
}

void semantic_segmentationOnnxInference::initialize_session_options(Ort::SessionOptions& session_options) {
...
std::vector<float> semantic_segmentationOnnxInference::preprocess_input(const std::vector<float>& raw_input) {
...
std::vector<float> semantic_segmentationOnnxInference::postprocess_output(const std::vector<float>& raw_output) {
...
bool semantic_segmentationOnnxInference::run_inference_tensorrt(const std::vector<float>& input_data, std::vector<float>& output_data) {
...
bool semantic_segmentationOnnxInference::run_inference_tensorrt_cuda(const float* input_data, size_t input_size, 
                                                              float* output_data, size_t output_size) {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
...
bool semantic_segmentationOnnxInference::enable_tensorrt_fp16() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool semantic_segmentationOnnxInference::enable_tensorrt_int8() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool semantic_segmentationOnnxInference::set_tensorrt_workspace_size(size_t size) {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool semantic_segmentationOnnxInference::clear_tensorrt_cache() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool semantic_segmentationOnnxInference::initialize_tensorrt_options(OrtTensorRTProviderOptions& trt_options) {
    trt_options.device_id = 0;
...
bool semantic_segmentationOnnxInference::create_tensorrt_cache_directory() {
    std::string command = "mkdir -p " + tensorrt_cache_path_;
    int result = system(command.c_str());
    return result == 0;
}

bool semantic_segmentationOnnxInference::validate_tensorrt_engine() {
    if (!tensorrt_enabled_) {
        return false;
    }
