...
__global__ void edge_detection_kernel(
    float** input_image,    int* width,    int* height,    float** edge_image, int num_elements
) {
...
edge_detectionKernel::edge_detectionKernel(cudaStream_t* stream)
    : stream_(stream) {
...
edge_detectionKernel::~edge_detectionKernel() {
    try {
        freeDeviceMemory();
...
bool edge_detectionKernel::initialize(const float** input, size_t input_size) {
...
bool edge_detectionKernel::process(const float** parameters, size_t param_size, float** output, size_t output_size) {
    if (!initialized_) {
        return false;
    }
    ...
    // Launch kernel
    edge_detection_kernel<<<num_blocks, block_size, 0, stream_ ? *stream_ : 0>>>(
        d_input_image_,        d_width_,        d_height_,        d_edge_image_, input_size_
    );
    ...
    if (!checkCudaError(status, "Copy edge_image from device")) {
        return false;
    }
...
bool edge_detectionKernel::allocateDeviceMemory() {
    cudaError_t status;
    ...
    status = cudaMalloc(&d_edge_image_, input_size_ * sizeof(float*));
    if (!checkCudaError(status, "Allocate edge_image memory")) {
        // Free previously allocated memory
        if (d_input_image_) {
            cudaFree(d_input_image_);
            d_input_image_ = nullptr;
...
void edge_detectionKernel::freeDeviceMemory() {
    if (d_input_image_) {
        cudaFree(d_input_image_);
        d_input_image_ = nullptr;
    }
...
bool edge_detectionKernel::checkCudaError(cudaError_t status, const std::string& context) {
    if (status != cudaSuccess) {
