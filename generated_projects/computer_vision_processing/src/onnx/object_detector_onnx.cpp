#include "object_detector_onnx.hpp"
#include <iostream>
#include <fstream>
#include <stdexcept>
#include <algorithm>
...
object_detectorOnnxInference::object_detectorOnnxInference(const std::string& model_path)
    : model_path_(model_path), device_type_("gpu") {
    
    // Set up environment
    env_ = Ort::Env(ORT_LOGGING_LEVEL_WARNING, "object_detector_onnx");
...
object_detectorOnnxInference::~object_detectorOnnxInference() {
    // Free CUDA memory
...
bool object_detectorOnnxInference::initialize() {
    try {
...
bool object_detectorOnnxInference::run_inference(const std::vector<float>& input_data, std::vector<float>& output_data) {
    try {
...
bool object_detectorOnnxInference::run_inference_cuda(const float* input_data, size_t input_size, 
...
void* object_detectorOnnxInference::allocate_cuda_memory(size_t size) {
    void* ptr = nullptr;
...
void object_detectorOnnxInference::free_cuda_memory(void* ptr) {
    if (ptr) {
        cudaFree(ptr);
...
bool object_detectorOnnxInference::copy_to_cuda(const std::vector<float>& host_data, void* cuda_ptr) {
    cudaError_t error = cudaMemcpy(cuda_ptr, host_data.data(), 
                                   host_data.size() * sizeof(float), cudaMemcpyHostToDevice);
    return error == cudaSuccess;
}

bool object_detectorOnnxInference::copy_from_cuda(void* cuda_ptr, std::vector<float>& host_data) {
    cudaError_t error = cudaMemcpy(host_data.data(), cuda_ptr, 
                                   host_data.size() * sizeof(float), cudaMemcpyDeviceToHost);
    return error == cudaSuccess;
}

void object_detectorOnnxInference::initialize_session_options(Ort::SessionOptions& session_options) {
...
std::vector<float> object_detectorOnnxInference::preprocess_input(const std::vector<float>& raw_input) {
...
std::vector<float> object_detectorOnnxInference::postprocess_output(const std::vector<float>& raw_output) {
...
bool object_detectorOnnxInference::run_inference_tensorrt(const std::vector<float>& input_data, std::vector<float>& output_data) {
...
bool object_detectorOnnxInference::run_inference_tensorrt_cuda(const float* input_data, size_t input_size, 
                                                              float* output_data, size_t output_size) {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
...
bool object_detectorOnnxInference::enable_tensorrt_fp16() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool object_detectorOnnxInference::enable_tensorrt_int8() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool object_detectorOnnxInference::set_tensorrt_workspace_size(size_t size) {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool object_detectorOnnxInference::clear_tensorrt_cache() {
    if (!tensorrt_enabled_) {
        std::cerr << "TensorRT is not enabled for this model" << std::endl;
        return false;
    }
...
bool object_detectorOnnxInference::initialize_tensorrt_options(OrtTensorRTProviderOptions& trt_options) {
    trt_options.device_id = 0;
...
bool object_detectorOnnxInference::create_tensorrt_cache_directory() {
    std::string command = "mkdir -p " + tensorrt_cache_path_;
    int result = system(command.c_str());
    return result == 0;
}

bool object_detectorOnnxInference::validate_tensorrt_engine() {
    if (!tensorrt_enabled_) {
        return false;
    }
