...
face_recognitionOnnxManager::face_recognitionOnnxManager() {
...
bool face_recognitionOnnxManager::initialize() {
    // Set model paths
    face_recognition_model_path_ = "models/face_recognition.onnx";
...
bool face_recognitionOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for face_recognition: " << e.what() << std::endl;
        return false;
    }
...
void face_recognitionOnnxManager::cleanup() {
    sessions_.clear();
}

bool face_recognitionOnnxManager::load_models() {
    ...
    try {
        auto session = std::make_unique<Ort::Session>(env_, face_recognition_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load face_recognition model: " << e.what() << std::endl;
        return false;
    }
...
bool face_recognitionOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool face_recognitionOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool face_recognitionOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
