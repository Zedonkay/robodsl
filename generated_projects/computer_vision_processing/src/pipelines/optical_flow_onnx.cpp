...
optical_flowOnnxManager::optical_flowOnnxManager() {
...
bool optical_flowOnnxManager::initialize() {
    // Set model paths
    optical_flow_model_path_ = "models/optical_flow.onnx";
...
bool optical_flowOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for optical_flow: " << e.what() << std::endl;
        return false;
    }
...
void optical_flowOnnxManager::cleanup() {
    sessions_.clear();
}

bool optical_flowOnnxManager::load_models() {
    Ort::SessionOptions session_options;
    session_options.SetIntraOpNumThreads(1);
    session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
    
    // Load optical_flow model
    try {
        auto session = std::make_unique<Ort::Session>(env_, optical_flow_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load optical_flow model: " << e.what() << std::endl;
        return false;
    }
...
bool optical_flowOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool optical_flowOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool optical_flowOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
