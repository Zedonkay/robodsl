...
depth_estimationOnnxManager::depth_estimationOnnxManager() {
...
bool depth_estimationOnnxManager::initialize() {
    // Set model paths
    depth_estimation_model_path_ = "models/depth_estimation.onnx";
...
bool depth_estimationOnnxManager::run_inference(const std::vector<float>& input_data, 
    ...
    try {
        ...
        std::cerr << "ONNX inference failed for depth_estimation: " << e.what() << std::endl;
        return false;
    }
...
void depth_estimationOnnxManager::cleanup() {
    sessions_.clear();
}

bool depth_estimationOnnxManager::load_models() {
    ...
    try {
        auto session = std::make_unique<Ort::Session>(env_, depth_estimation_model_path_.c_str(), session_options);
        sessions_.push_back(std::move(session));
    } catch (const Ort::Exception& e) {
        std::cerr << "Failed to load depth_estimation model: " << e.what() << std::endl;
        return false;
    }
...
bool depth_estimationOnnxManager::setup_io_info() {
    if (sessions_.empty()) {
        return false;
    }
...
bool depth_estimationOnnxManager::prepare_input_tensors(const std::vector<float>& input_data,
                                                       std::vector<Ort::Value>& input_tensors) {
...
bool depth_estimationOnnxManager::extract_output_tensors(const std::vector<Ort::Value>& output_tensors,
                                                        std::vector<float>& output_data) {
