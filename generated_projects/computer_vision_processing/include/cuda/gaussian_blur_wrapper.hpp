...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for gaussian_blur
 * 
 ...
 */
class gaussian_blurWrapper {
public:
    /**
     * @brief Construct a new gaussian_blur Wrapper object
     * 
     ...
     */
    explicit gaussian_blurWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating gaussian_blurWrapper");
        }
    }
    
    /**
     * @brief Destroy the gaussian_blur Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~gaussian_blurWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying gaussian_blurWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    gaussian_blurWrapper(const gaussian_blurWrapper&) = delete;
    gaussian_blurWrapper& operator=(const gaussian_blurWrapper&) = delete;
    
    // Allow move constructor and assignment
    gaussian_blurWrapper(gaussian_blurWrapper&& other) noexcept
        ...
        d_blurred_image_ = other.d_blurred_image_;
        other.d_blurred_image_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    gaussian_blurWrapper& operator=(gaussian_blurWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized gaussian_blurWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            ...
            cudaError_t status = cudaMalloc(&d_blurred_image_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_blurred_image_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_blurred_image_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_blurred_image_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_blurred_image_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_blurred_image_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        gaussian_blur_kernel<<<grid_size, block_size, 4096, stream_>>>(
            d_input_image_,            d_width_,            d_height_,            d_channels_,            d_blurred_image_            num_elements
        );
...
inline std::unique_ptr<gaussian_blurWrapper> creategaussian_blurWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<gaussian_blurWrapper>(node, stream);
}
