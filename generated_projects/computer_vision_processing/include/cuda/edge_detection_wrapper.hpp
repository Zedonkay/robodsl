...
// CUDA kernel header
#include "cuda/edge_detection_kernel.cuh"

namespace robodsl {

/**
 * @brief ROS2-compatible CUDA kernel wrapper for edge_detection
 * 
 ...
 */
class edge_detectionWrapper {
public:
    /**
     * @brief Construct a new edge_detection Wrapper object
     * 
     ...
     */
    explicit edge_detectionWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating edge_detectionWrapper");
        }
    }
    
    /**
     * @brief Destroy the edge_detection Wrapper object
     * 
     * Frees all allocated device memory and CUDA resources.
     */
    ~edge_detectionWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying edge_detectionWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    edge_detectionWrapper(const edge_detectionWrapper&) = delete;
    edge_detectionWrapper& operator=(const edge_detectionWrapper&) = delete;
    
    // Allow move constructor and assignment
    edge_detectionWrapper(edge_detectionWrapper&& other) noexcept
        ...
        d_edge_image_ = other.d_edge_image_;
        other.d_edge_image_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    edge_detectionWrapper& operator=(edge_detectionWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized edge_detectionWrapper on device %d: %s", 
...
private:
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            ...
            cudaError_t status = cudaMalloc(&d_edge_image_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_edge_image_")) {
                return false;
            }
    ...
    void freeDeviceMemory() {
        ...
        if (d_edge_image_) {
            cudaFree(d_edge_image_);
            d_edge_image_ = nullptr;
        }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_edge_image_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_edge_image_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_edge_image_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_edge_image_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        edge_detection_kernel<<<grid_size, block_size, 2048, stream_>>>(
            d_input_image_,            d_width_,            d_height_,            d_edge_image_            num_elements
        );
...
inline std::unique_ptr<edge_detectionWrapper> createedge_detectionWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<edge_detectionWrapper>(node, stream);
}
