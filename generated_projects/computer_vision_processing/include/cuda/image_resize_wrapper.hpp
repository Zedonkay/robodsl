...
#ifndef IMAGE_RESIZE_KERNEL_HPP
#define IMAGE_RESIZE_KERNEL_HPP
...
/**
 * @brief ROS2-compatible CUDA kernel wrapper for image_resize
 * 
 ...
 */
class image_resizeWrapper {
public:
    /**
     * @brief Construct a new image_resize Wrapper object
     * 
     ...
     */
    explicit image_resizeWrapper(
        rclcpp::Node* node = nullptr,
        cudaStream_t stream = nullptr
    ) : node_(node), stream_(stream), device_id_(0), initialized_(false), allocated_size_(0) {
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Creating image_resizeWrapper");
        }
    }
    
    /**
     * @brief Destroy the image_resize Wrapper object
     * 
    ...
    ~image_resizeWrapper() {
        freeDeviceMemory();
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), "Destroying image_resizeWrapper");
        }
    }
    
    // Delete copy constructor and assignment operator
    image_resizeWrapper(const image_resizeWrapper&) = delete;
    image_resizeWrapper& operator=(const image_resizeWrapper&) = delete;
    
    // Allow move constructor and assignment
    image_resizeWrapper(image_resizeWrapper&& other) noexcept
        ...
        d_input_image_ = other.d_input_image_;
        other.d_input_image_ = nullptr;
        d_input_width_ = other.d_input_width_;
        other.d_input_width_ = nullptr;
        d_input_height_ = other.d_input_height_;
        other.d_input_height_ = nullptr;
        d_channels_ = other.d_channels_;
        other.d_channels_ = nullptr;
        d_output_image_ = other.d_output_image_;
        other.d_output_image_ = nullptr;
        d_output_width_ = other.d_output_width_;
        other.d_output_width_ = nullptr;
        d_output_height_ = other.d_output_height_;
        other.d_output_height_ = nullptr;
        
        other.initialized_ = false;
        other.allocated_size_ = 0;
    }
    
    image_resizeWrapper& operator=(image_resizeWrapper&& other) noexcept {
        if (this != &other) {
            freeDeviceMemory();
    ...
    bool initialize(int device_id = 0) {
        ...
        if (node_) {
            RCLCPP_INFO(node_->get_logger(), 
                "Initialized image_resizeWrapper on device %d: %s", 
...
private:
    ...
    int* d_input_width_ = nullptr;  //!< Device memory for input_width
    int* d_input_height_ = nullptr;  //!< Device memory for input_height
    int* d_channels_ = nullptr;  //!< Device memory for channels
    float** d_output_image_ = nullptr;  //!< Device memory for output_image
    int* d_output_width_ = nullptr;  //!< Device memory for output_width
    int* d_output_height_ = nullptr;  //!< Device memory for output_height
    ...
    bool allocateDeviceMemory(size_t input_size, size_t output_size) {
        ...
        if (allocated_size_ == 0) {
            cudaError_t status = cudaMalloc(&d_input_image_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_input_image_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_input_width_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_input_width_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_input_height_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_input_height_")) {
                return false;
            }
            ...
            cudaError_t status = cudaMalloc(&d_output_image_, input_size * sizeof(float*));
            if (!checkCudaError(status, "cudaMalloc for d_output_image_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_output_width_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_output_width_")) {
                return false;
            }
            cudaError_t status = cudaMalloc(&d_output_height_, input_size * sizeof(int));
            if (!checkCudaError(status, "cudaMalloc for d_output_height_")) {
                return false;
            }
    ...
    bool copyToDevice(const std::vector<float>& input_data) {
        ...
        cudaError_t status = cudaMemcpy(d_input_image_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_input_image_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_input_width_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_input_width_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_input_height_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_input_height_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(d_output_image_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_output_image_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_output_width_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_output_width_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(d_output_height_, input_data.data(), 
                                       input_data.size() * sizeof(float), cudaMemcpyHostToDevice);
        if (!checkCudaError(status, "cudaMemcpy H2D for d_output_height_")) {
            return false;
        }
    ...
    bool copyFromDevice(std::vector<float>& output_data) {
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_input_image_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_input_image_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_input_width_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_input_width_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_input_height_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_input_height_")) {
            return false;
        }
        ...
        cudaError_t status = cudaMemcpy(output_data.data(), d_output_image_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_output_image_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_output_width_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_output_width_")) {
            return false;
        }
        cudaError_t status = cudaMemcpy(output_data.data(), d_output_height_, 
                                       allocated_size_ * sizeof(float), cudaMemcpyDeviceToHost);
        if (!checkCudaError(status, "cudaMemcpy D2H for d_output_height_")) {
            return false;
        }
    ...
    bool launchKernel(size_t num_elements) {
        ...
        // Launch kernel
        image_resize_kernel<<<grid_size, block_size, 2048, stream_>>>(
            d_input_image_,            d_input_width_,            d_input_height_,            d_channels_,            d_output_image_,            d_output_width_,            d_output_height_            num_elements
        );
...
inline std::unique_ptr<image_resizeWrapper> createimage_resizeWrapper(
    rclcpp::Node* node = nullptr,
    cudaStream_t stream = nullptr) {
    return std::make_unique<image_resizeWrapper>(node, stream);
}

} // namespace robodsl

#endif // IMAGE_RESIZE_KERNEL_HPP
