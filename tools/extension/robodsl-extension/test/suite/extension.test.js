...
suite('RoboDSL Extension Test Suite', () => {
    ...
    test('ROS types are available in completions', async () => {
        ...
        if (completions && completions.items.length > 0) {
            const hasRosType = completions.items.some(item => item.label.toString().startsWith('std_msgs/'));
    ...
    test('C++ types are available in completions', async () => {
        ...
        const completions = await vscode.commands.executeCommand('vscode.executeCompletionItemProvider', doc.uri, pos);
        if (completions && completions.items.length > 0) {
            const hasCppType = completions.items.some(item => item.label.toString().startsWith('std::'));
