...
#include "simple_test_kernel.cuh"
#include <algorithm>
#include <stdexcept>
...
simple_testKernel::simple_testKernel(cudaStream_t* stream)
    : stream_(stream) {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("simple_testKernel"), 
                "Initialized CUDA device: %s (Compute %d.%d)", 
                props.name, props.major, props.minor);
}

simple_testKernel::~simple_testKernel() {
    try {
        freeDeviceMemory();
    } catch (const std::exception& e) {
        RCLCPP_ERROR(rclcpp::get_logger("simple_testKernel"), 
                    "Error in destructor: %s", e.what());
    }
}

bool simple_testKernel::initialize(const std::vector<float*>& input) {
    if (input.empty()) {
    ...
    if (!checkCudaError(status, "Copy data to device")) {
        return false;
    }
...
std::vector<float*> simple_testKernel::process(const KernelParameters& parameters) {
    if (!initialized_) {
    ...
    // Launch kernel
    simple_test_kernel<<<num_blocks, block_size, 0, stream_ ? *stream_ : 0>>>(
        d_data_,        d_result_    );
...
bool simple_testKernel::allocateDeviceMemory() {
    cudaError_t status;
    
    // Allocate memory for all parameters
    status = cudaMalloc(&d_data_, input_size_ * sizeof(float*));
    if (!checkCudaError(status, "Allocate data memory")) {
...
void simple_testKernel::freeDeviceMemory() {
    if (d_data_) {
        cudaFree(d_data_);
        d_data_ = nullptr;
    }
...
bool simple_testKernel::checkCudaError(cudaError_t status, const std::string& context) {
    if (status != cudaSuccess) {
